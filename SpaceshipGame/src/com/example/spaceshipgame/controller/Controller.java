...
public class Controller {
		...
		for (Player player : gameState.players)
			synchronized (player) {
				...
				if (player.moveState.movingUp() == MoveState.State.ACCELERATE) {
					...
					if (player.spaceship.getVelocity() >= player.spaceship
							.getMaxVelocity()) {

						player.spaceship.setVelocity(player.spaceship
								.getMaxVelocity());
				...
				player.getSpaceship().validatePosition(new DoublePoint(0, 0),
						gameState.map.size, gameState.map.MARGIN);

				for (Element element : player.elements)

					if (element instanceof Missile) {
						element.moveAhead(time);
						if (!element.checkPosition(new DoublePoint(0, 0),
								gameState.map.size))
		...
		synchronized (gameState.currentInstancePlayer) {
			...
			if (player.moveState.attacking() == MoveState.State.ENABLE) {
				...
				if (player.canAttack() && player.hasAmmunitionLeft()) {
					...
					DoublePoint missilePosition = new DoublePoint(player
							.getSpaceship().getPosition());

					DoublePoint missileShift = new DoublePoint();
					missileShift.setByAngleAndLength(player.getSpaceship()
							.getRotation(),
							player.getSpaceship().getHeigt() / 2.0);
	...
	public void showToastMessage(final String text) {
		gameActivity.runOnUiThread(new Runnable() {
			public void run() {
				Toast toast = Toast.makeText(getGameContext(), text,
						Toast.LENGTH_SHORT);
				toast.show();
